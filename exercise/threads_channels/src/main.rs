...
fn main() {
    ...
    let (tx, rx) = channel::unbounded();
    let rx2 = rx.clone();
    let vals = vec![16, 32, 64, 128, 256];
    for val in vals {
        println!("Main thread: {}", val);
        tx.send(val)
            .expect("Channel should be able to receive values!");
    }
    let handle_a = thread::spawn(move || {
        println!("Child A: {}", rx.recv().unwrap());
        sleep_ms(0);
        println!("Child A: {}", rx.recv().unwrap());
        sleep_ms(500);
        println!("Child A: {}", rx.recv().unwrap());
    ...
    let handle_b = thread::spawn(move || {
        println!("Child B: {}", rx2.recv().unwrap());
        sleep_ms(300);
        println!("Child B: {}", rx2.recv().unwrap());
    });
    ...
    handle_a
        .join()
        .expect("Child A: should be able to join main thread!");
    handle_b
        .join()
        .expect("Child B: should be able to join main thread!");
